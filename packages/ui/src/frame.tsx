...
function FramePanel({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'not-has-[table]:bg-card not-has-[table]:rounded-xl not-has-[table]:border not-has-[table]:p-5 not-has-[table]:shadow-xs relative bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-xl)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] has-[table]:before:hidden dark:bg-clip-border dark:before:shadow-[0_-1px_--theme(--color-white/8%)]',
        className,
      )}
      {...props}
    />
